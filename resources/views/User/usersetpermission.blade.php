    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
      ...
      <div class="flex justify-end">
        ...
        <tbody>
            ...
            @php
    // گرفتن permissionهای غیرمستقیم از طریق role
    $permissionsViaRoles = $user->getPermissionsViaRoles();

    // ترکیب permissionهای مستقیم و غیرمستقیم و حذف تکراری‌ها
    $allPermissions = $user->permissions->merge($permissionsViaRoles)->unique('name');
@endphp

@foreach($allPermissions as $userpermission)
    <a href="{{ route('usersetrole', $user->id) }}" 
       class="bg-blue-500 hover:bg-blue-700 text-white font-medium py-[3px] px-[6px] text-xs border border-blue-700 rounded">
        {{ $userpermission->name }}
    </a> |
@endforeach
    ...
    <form class="max-w-sm mx-auto" action="{{route('storeusersetpermission',$user->id)}}" method="post">
  @csrf
...
<div class="max-w-md mx-auto mt-10">
  <label for="skills" class="block mb-2 text-sm font-medium text-gray-700">ُ<h1>Select permissions</h1></label>
