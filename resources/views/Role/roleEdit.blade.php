...
<form class="max-w-sm mx-auto" action="{{route('roleUpdate',$role->id)}}" method="post">
  @csrf
  @method('PUT')
  <h1>Edite role</h1>
  <br>
  <div class="mb-5">
    ...
    <input type="text" value="{{$role->name}}" name="name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"  required />
  </div>
 <br>
 <style>
  select[multiple] {
...
<div class="max-w-md mx-auto mt-10">
  ...
  <option    @foreach($role->permissions as $rolepermission) @if($permission->id==$rolepermission->id) selected @endif    @endforeach
